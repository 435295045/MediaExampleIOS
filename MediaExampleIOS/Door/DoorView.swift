...
class DoorView: UIViewController, CameraSessionDelegate {
    let representation: DoorViewRepresentation!;
    ...
    init(representation: DoorViewRepresentation, initiator: Bool, clientId: String) {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if (!initiator) {
            ...
            /*if !MediaSDK.Door().call(clientId: clientId) {
        ...
        //订阅视频
        MediaSDK.door().consume(clientId: nil);
    }
    ...
    override func viewDidDisappear(_ animated: Bool) {
        ...
        //移除监听（必须）
        MediaSDK.door().removeListener(listener: self)
        //释放呼叫（必须）
...
extension DoorView: DoorListener {
    func onDoorListener(event: DoorState) {
        if case let DoorState.offerAck(code: code, data: data) = event {
            print("code:\(code)  data:\(data)");
        } else if case let DoorState.answer(clientId: clientId) = event {
            MediaSDK.door().producer(video: true);
        } else if case let DoorState.answerAck(code: code, data: data) = event {
            if code == 200 {
                MediaSDK.door().producer(video: false);
            }
        } else if case let DoorState.hangUp(clientId: clientId, code: code, data: data) = event {
        ...
        } else if case let DoorState.producer(clientId: clientId) = event {
            MediaSDK.door().consume(clientId: nil);
        } else if case let DoorState.producerAck(code: code, data: data) = event {
            print("code:\(code)  data:\(data)");
        } else if case let DoorState.consumeAck(code: code, data: data) = event {
            print("code:\(code)  data:\(data)");
        } else if case let DoorState.media(peer: peer, stream: stream) = event {
            if (stream == nil) {
                self.peer = peer;
...
extension DoorView: OnClickListener {
    func muteListener() {
        if peer != nil {
